...
// switchV2Cmd represents the new switch command with improved configuration
var switchV2Cmd = &cobra.Command{
	Use:   "switch [provider]",
...
func newSwitchV2Cmd() *cobra.Command {
	return switchV2Cmd
}

func runSwitchV2(cmd *cobra.Command, args []string) error {
	...
	// Get provider name
	providerName, err := getProviderNameV2(args, cfg, verbose)
	if err != nil {
		return err
	}
	...
	// Switch provider
	if err := switchProviderV2(tomlManager, providerName); err != nil {
		return err
	}
	...
	if !quiet {
		displaySwitchSuccessV2(provider, verbose)
	}

	return nil
}

func getProviderNameV2(args []string, cfg *config.CFLIPConfig, verbose bool) (string, error) {
	if len(args) > 0 {
		return args[0], nil
	}

	return promptProviderSelectionV2(cfg)
}

func promptProviderSelectionV2(cfg *config.CFLIPConfig) (string, error) {
...
func switchProviderV2(tomlManager *config.TOMLManagerV2, providerName string) error {
...
func displaySwitchSuccessV2(provider *config.ProviderInfo, verbose bool) {
